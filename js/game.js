...
let gameState = {
    score: 0,
    mushroomHistory: [],
    playerPosition: { x: 25, y: canvas.height - config.pipeHeight - config.playerSize },
    jumping: false,
    jumpVelocity: 0,
    gameRunning: false,
    lastMushroomTime: 0,
...
function updateBackground() {
    ...
    if (bushImage.complete) {
        bushes.forEach(bush => {
            ...
            bush.y = canvas.height - config.floorHeight - config.bushSize.height + 7.5;
        });
    }
...
// Draw pipe
function drawPipe() {
    ctx.drawImage(pipeImage, 20, canvas.height - config.pipeHeight -40, config.pipeWidth, config.pipeHeight);
}
...
function startGame() {
    ...
    setTimeout(() => {
        const moveInterval = setInterval(() => {
            ...
            if (gameState.playerPosition.x >= 50) { // Move to x position 150
...
function drawInitialScene() {
    ...
    if (treeImage.complete) {
        trees.forEach(tree => {
            ctx.drawImage(treeImage, tree.x, tree.y, config.treeSize.width, config.treeSize.height+17);
            tree.y = canvas.height - config.floorHeight - config.treeSize.height;
        });
    }
    ...
    if (bushImage.complete) {
        bushes.forEach(bush => {
            ctx.drawImage(bushImage, bush.x, bush.y, config.bushSize.width, config.bushSize.height+7.5);
            bush.y = canvas.height - config.floorHeight - config.bushSize.height ;
        });
    }

    
    ctx.drawImage(player1Image, gameState.playerPosition.x-7, gameState.playerPosition.y-40, config.playerSize, config.playerSize);
}
...
Promise.all(essentialImages.map(img => new Promise(resolve => img.onload = resolve)))
    .then(() => initGame());
